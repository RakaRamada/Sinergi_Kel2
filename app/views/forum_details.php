...
// Data dikirim dari ForumController: $forum_info, $forum_members, $forum_media, $forum_documents
?>
    ...
    <div class="p-6 space-y-6 pb-6">
        ...
        <div class="flex flex-col items-center">
            ...
            <div class="flex items-center space-x-2">
                <h2 class="text-2xl font-bold text-gray-900"><?= htmlspecialchars($forum_info['nama_forum']) ?></h2>
            ...
            <?php 
    $tgl_dibuat = $forum_info['created_at'] ?? '';
...
?>
        ...
        <div class="border-t border-gray-100 pt-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-gray-800 font-bold text-lg"><?= count($forum_members) ?> Anggota</h3>
            ...
            <div class="space-y-1">
                ...
                <?php $is_member_admin = ($member['user_id'] == $forum_info['created_by_user_id']); ?>
        ...
        <div class="border-t border-gray-100 pt-6">
            <h3 class="font-bold text-gray-800 mb-4">Media (<?= count($forum_media) ?>)</h3>
            <?php if (!empty($forum_media)): ?>
            <div class="grid grid-cols-3 md:grid-cols-4 gap-2">
                <?php foreach ($forum_media as $media): ?>
                <?php $media_url = '/Sinergi/public/uploads/forum_files/' . htmlspecialchars($media['file_path']); ?>
        ...
        <div class="border-t border-gray-100 pt-6">
            <h3 class="font-bold text-gray-800 mb-4">Dokumen (<?= count($forum_documents) ?>)</h3>
            <?php if (!empty($forum_documents)): ?>
            <div class="space-y-2">
                <?php foreach ($forum_documents as $doc): ?>
                <?php $doc_url = '/Sinergi/public/uploads/forum_files/' . htmlspecialchars($doc['file_path']); ?>
    ...
    <div
        ...
        <div class="mt-2">
            <p class="text-sm text-gray-500" id="confirmMessage">
                Apakah Anda yakin ingin menambahkan user ini ke forum?
            </p>
        </div>
...
const forumId = <?= (int)$forum_info['forum_id'] ?>;
...
function fetchCandidates(query) {
    fetch(`index.php?page=api-search-candidates&forum_id=${forumId}&q=${encodeURIComponent(query)}`)
...
function submitAddMember(userId, userName) {
    ...
    confirmMessage.innerHTML =
        `Apakah Anda yakin ingin menambahkan <strong>${escapeHtml(userName)}</strong> ke forum ini?`;
...
function openKickModal(userId, userName) {
    // Update teks
    kickMessageText.innerHTML = `Yakin ingin mengeluarkan <strong>${userName}</strong> dari forum?`;

    // Update Link Href pada tombol "Ya"
    // Format URL: index.php?page=kick-member&forum_id=XXX&user_id=YYY&name=ZZZ
    // forumId diambil dari variabel global PHP di atas (pastikan ada const forumId = ...)
    const link =
        `index.php?page=kick-member&forum_id=${forumId}&user_id=${userId}&name=${encodeURIComponent(userName)}`;
