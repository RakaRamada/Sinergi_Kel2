...
// Variabel $currentTab, $query, $group_results, dan $user_results
...
<main class="col-span-6 border-r border-gray-200">
    ...
    <div class="p-4 border-b border-gray-200 sticky top-0 bg-white/80 backdrop-blur-sm z-10">
        <form action="index.php" method="GET" class="relative">
            ...
            <input type="text" name="q" placeholder="Cari group atau orang..." value="<?= htmlspecialchars($query) ?>"
    ...
    <div class="flex border-b border-gray-200">
        <a href="index.php?page=search&tab=group&q=<?= htmlspecialchars($query) ?>"
            class="flex-1 text-center py-3 font-semibold hover:bg-gray-100 <?php if ($currentTab === 'group') echo 'border-b-2 border-black text-black font-bold'; else echo 'text-gray-500'; ?>">
            Group
        </a>
    ...
    <div>
        ...
        <div class="p-4 border-b border-gray-200 hover:bg-gray-50">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <img src="<?= $image_path ?>" alt="Avatar Group" class="w-12 h-12 rounded-full mr-4 object-cover">
                    <div>
                        <p class="font-bold text-gray-800"><?= htmlspecialchars($group['nama_group']) ?></p>
                        <p class="text-sm text-gray-600 mt-1 truncate"><?= $deskripsi ?></p>
                    </div>
                </div>

                <a href="index.php?page=join-group&group_id=<?= $group['group_id'] ?>"
        ...
        <?php else: ?>
        <div class="p-4 text-center text-gray-500">
            <p>Tidak ada group yang cocok dengan "<?= htmlspecialchars($query) ?>".</p>
        </div>
        <?php endif; ?>
