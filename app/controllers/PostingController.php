...
function showPostDetail(){
    ...
    // === [TAMBAHAN PENTING] FIX CLOB ===
    // Ini kode yang mengubah Object CLOB menjadi String biasa
    if (isset($post['KONTEN']) && is_object($post['KONTEN'])) {
        $post['KONTEN'] = $post['KONTEN']->load();
    }
    // ===================================
    ...
    // Fix Gambar Postingan
    $post['POST_IMAGE'] = $post['POST_IMAGE'] ?? '';
    if (!empty($post['POST_IMAGE']) && strpos($post['POST_IMAGE'], '/') === false) {
        $post['POST_IMAGE'] = '/Sinergi/public/uploads/posts/' . $post['POST_IMAGE'];
    }
...
$sql_comments = "
    ...
    while ($row_c = oci_fetch_assoc($stmt_c)) {
        ...
        // === [FIX JUGA UNTUK KOMENTAR] ===
        if (isset($c['ISI_KOMEN']) && is_object($c['ISI_KOMEN'])) {
            $c['ISI_KOMEN'] = $c['ISI_KOMEN']->load();
        }
        // =================================
    ...
    $sql_replies = "
        ...
        while ($row_r = oci_fetch_assoc($stmt_r)) {
            ...
            // FIX CLOB REPLY
            if (isset($r['ISI_KOMEN']) && is_object($r['ISI_KOMEN'])) {
                $r['ISI_KOMEN'] = $r['ISI_KOMEN']->load();
            }
