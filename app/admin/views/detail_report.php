...
<main class="flex-1 p-6 ml-0 lg:ml-20">
  <div class="max-w-4xl mx-auto">
    <div class="mb-4">
      <a href="/Sinergi/index.php?page=admin-dashboard" class="inline-flex items-center gap-2 text-sm text-gray-600 hover:underline">
    ...
    <div class="bg-white border rounded-lg shadow-sm p-6 mb-6">
      <div class="flex items-start gap-4 mb-4">
        <img src="<?= htmlspecialchars($ownerAvatar) ?>" alt="avatar" class="w-12 h-12 rounded-full object-cover border">
        <div>
      ...
      <?php if ($postImage): ?>
      <div class="mb-4">
        <img src="<?= htmlspecialchars($postImage) ?>" alt="post image" class="w-full rounded-lg border max-h-96 object-contain">
      </div>
      <?php endif; ?>
    ...
    <div class="bg-white border rounded-lg shadow-sm p-6">
      ...
      <div class="space-y-3 mb-6">
        <div class="flex items-start gap-3">
          <img src="<?= htmlspecialchars($reporterAvatar) ?>" alt="pelapor" class="w-10 h-10 rounded-full border">
          <div>
            <p class="text-sm text-gray-600">Pelapor:</p>
            <p class="font-semibold">@<?= htmlspecialchars($reporterUsername) ?> (<?= htmlspecialchars($reporterName) ?>)</p>
          </div>
        </div>
      ...
      <div class="mt-6 flex flex-wrap gap-3">
        <?php if ($postId): ?>
          <button onclick="deletePost(<?= $reportId ?>)" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition">
            🗑️ Hapus Postingan
          </button>
        <?php endif; ?>
        ...
        <button onclick="markResolved(<?= $reportId ?>)" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">
          ✓ Tandai Selesai
        </button>

        <button onclick="markRejected(<?= $reportId ?>)" class="px-4 py-2 bg-gray-400 text-white rounded hover:bg-gray-500 transition">
          ✗ Tolak Laporan
        </button>
      </div>
      <?php else: ?>
        <div class="bg-gray-50 p-3 rounded">
          <p class="text-sm text-gray-600">Laporan ini sudah diproses dengan status: <strong class="<?= $statusColor ?>"><?= strtoupper($status) ?></strong></p>
        </div>
      <?php endif; ?>
    </div>
  </div>
