...
<main class="flex-1 p-6 ml-0 lg:ml-20">
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <<<<<<< HEAD <h1 class="text-2xl font-semibold">Report</h1>
        ...
        <div class="bg-white border rounded-lg shadow-sm p-4">
                    ...
                    <tbody id="reports-tbody" class="divide-y">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-4 pr-6 align-top">
                                ...
                                <a href="/Sinergi/app/admin/views/detail_report.php?post_id=<?= $i ?>"
                                    class="text-blue-600 hover:underline">Postingan →</a>
                            </td>
                            ...
                            <td class="py-4 pr-6 align-top text-right">
                                ...
                                <div class="relative inline-block text-left">
                                    <button
                                        class="action-more inline-flex items-center justify-center w-10 h-8 rounded-full hover:bg-gray-100"
                                    ...
                                    <div
                                        class="action-menu hidden absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-50">
                                        <button data-action="view" data-id="<?= $i ?>"
                                            class="w-full text-left px-4 py-2 hover:bg-gray-50">Lihat Postingan</button>
                                        <button data-action="delete" data-id="<?= $i ?>"
                                            class="w-full text-left px-4 py-2 hover:bg-gray-50 text-red-600">Hapus
                                            Postingan</button>
                                        <button data-action="block" data-id="<?= $i ?>"
                                            class="w-full text-left px-4 py-2 hover:bg-gray-50">Blokir User</button>
                                        <button data-action="mark_done" data-id="<?= $i ?>"
                                            class="w-full text-left px-4 py-2 hover:bg-gray-50">Tandai Selesai</button>
                                        <button data-action="mark_pending" data-id="<?= $i ?>"
                                            class="w-full text-left px-4 py-2 hover:bg-gray-50">Tandai Pending</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
            ...
            <div class="mt-4 text-center">
                <button id="load-more" class="px-4 py-2 bg-black text-white rounded-full">Tampilkan lebih banyak
                    ▾</button>
                =======
...
<<<<<<< HEAD <!-- small inline script for dropdown and actions (frontend-only) -->
    <script>
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    tbody.innerHTML =
                        `<tr><td colspan="7" class="py-8 text-center text-red-500 font-medium">Error Server:<br>${data.message || 'Unknown error'}</td></tr>`;
                }
            })
            .catch(err => {
                console.error('Fetch error:', err);
                tbody.innerHTML =
                    `<tr><td colspan="7" class="py-8 text-center text-red-500">Gagal terhubung ke server.<br><span class="text-xs text-gray-500">${err.message}</span></td></tr>`;
            });
    }
    ...
    function renderReports() {
        ...
        if (filtered.length === 0) {
            tbody.innerHTML =
                `<tr><td colspan="7" class="py-8 text-center text-gray-500">Pencarian tidak ditemukan.</td></tr>`;
            return;
        }
        ...
        reportsToShow.forEach((report, index) => {
            ...
            if (report.POST_OWNER_ID) {
                if (report.IS_OWNER_BANNED) {
                    banButton =
                        `<span class="block w-full text-left px-4 py-2 text-xs text-gray-400 italic bg-gray-50">🚫 User sudah dibanned</span>`;
                } else {
                    banButton = `
